...
const checkPrice = async (_exchange, _token0, _token1) => {
  ...
  const uFPrice = Number(uPrice).toFixed(units) // This line returns 0
  const sFPrice = Number(sPrice).toFixed(units) // This line returns 0
  console.log(`uFPrice: ${uFPrice}`)
  console.log(`uFPrice: ${uFPrice}`)
...
const determineProfitability = async (_routerPath, _token0Contract, _token0, _token1) => {
  ...
  try {
    ...
    console.log(`Estimated amount of WETH needed to buy enough IMX on ${exchangeToBuy}\t\t| ${ethers.formatUnits(estimate[0], 'ether')}`)
    console.log(`Estimated amount of WETH returned after swapping IMX on ${exchangeToSell}\t| ${ethers.formatUnits(result[1], 'ether')}\n`)
